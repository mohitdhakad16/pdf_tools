...
const AddPageNo = () => {
    ...
    // 1) selecting the pdf file to displaying it over the dom screen
    const handleFileInputChange = (e) => {
        const files = Array.from(e.target.files);   // converting the selected pdf into array and then mapping through it then return the image url to showcase the image 
    ...
    // 7) Logic for adding numbers to the pdf pages

    const handleDownloadPdf = async () => {
        const pdfName = editName.trim() !== "" ? editName : `${new Date().getTime()}.pdf`;

        const pdfBytes = await fetch(displayedFiles[0].src).then((res) => res.arrayBuffer()); // Fetch the PDF as array buffer
    ...
    return (
        <div className={`w-full p-4 text-center border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-${mode} dark:border-gray-700`}>
            <h5 className={`mb-2 text-3xl font-bold text-${mode === 'gray-800' ? 'gray-50' : 'gray-800'} dark:text-gray`}>Add page numbers</h5>
            <p className="mb-5 text-base text-gray-500 sm:text-lg">
                Free and easy to use tool to add page numbers to PDF files
            </p>
            ...
            <div className="py-4 text-center">
                ...
                <button className="inline-block w-40 bg-yellow-300 border border-yellow-500 rounded-full p-3 cursor-pointer" onClick={handleSelectFilesClick}>
                    ...
                    <input type="file" className="hidden" ref={fileInputRef} onChange={handleFileInputChange} accept=".pdf" />
                </button>
