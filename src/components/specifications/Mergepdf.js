...
import { PDFDocument, StandardFonts } from 'pdf-lib';     // using  external library that allow you to merge pdf
...
const ImagesToPdf = () => {
  ...
  // 1) selecting the pdfs and displaying it over the dom screen
  const handleFileInputChange = (e) => {
    const files = Array.from(e.target.files);   // converting the selected pdf files into array and then mapping through it 
  ...
  // 2) Deleting the pdf which u don't want to merge into ur pdf
  const handleDelete = (index) => {
    const updatedDisplayedFiles = [...displayedFiles];  // removing the array element by spread operator
    updatedDisplayedFiles.splice(index, 1); // Remove the selected pdf 
  ...
  // 7) Logic for merging the pdf's into single pdf file
  ...
  return (
    <div className={`w-full p-4 text-center border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-${mode} dark:border-gray-700`}>
      <h5 className={`mb-2 text-3xl font-bold text-${mode === 'gray-800' ? 'gray-50' : 'gray-800'} dark:text-gray`}>Merge PDF</h5>
      ...
      <div className="py-4 text-center">
        ...
        <button className="inline-block w-40 bg-yellow-300 border border-yellow-500 rounded-full p-3 cursor-pointer" onClick={handleSelectFilesClick}>
          ...
          <input type="file" className="hidden" ref={fileInputRef} onChange={handleFileInputChange} multiple accept=".pdf" />
        </button>
        ...
        {displayedFiles.length > 0 && (
          <div className="mt-4">
            ...
            <ul className="mt-2 flex justify-center items-baseline flex-wrap">
                ...
                <div className={`imagebox mx-2 my-2`} key={index}>
                  ...
                  <div className={`imgSection p-2 flex flex-col justify-center border-${border} border-2`}>
                    <div className={`fileImg p-2 mx-2  cursor-pointer`}>
                      <img src={colorPDF} alt={file.file.name} />
                    </div>
                  </div>
                ...
                {showDownloadButton && (
                  <>
                    ...
                    <div className="flex justify-center mt-4">
                      <button onClick={handleMergePdf} className="my-2 mx-1 px-4  py-2 bg-yellow-500 font-medium text-black rounded-md text-center">
                        Download PDF
                      </button>
